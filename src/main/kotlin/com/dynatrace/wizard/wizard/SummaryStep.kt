...
class SummaryStep {
    ...
    fun updateSummary(
        ...
        val configSummary = buildString {
            ...
            appendLine("ANR reporting:            ${if (config.anrReporting) "Enabled (Android 11+ only)" else "Disabled"}")
            appendLine("Native crash reporting:   ${if (config.nativeCrashReporting) "Enabled (Android 11+ only)" else "Disabled"}")
